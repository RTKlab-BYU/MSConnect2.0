...
- QC aggregates: `/api/qc/overview/` and `/api/qc/details/` expose HYE pair-level pseudo-project QC metrics today and a stable empty scaffold for `program=prtc`.
- Uploads: `/api/direct-uploads/` creates a direct-upload session with same-origin chunk URLs. Browser uploads stream to Django, then `/api/direct-uploads/<id>/complete/` records the completed `RawFile` after the file is staged into managed raw storage.
