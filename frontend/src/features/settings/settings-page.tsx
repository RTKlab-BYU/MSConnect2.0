...
import {
  ...
  fetchDeploymentReleases, createDeploymentRelease, promoteDeploymentRelease, rolloutDeploymentRelease, verifyDeploymentRelease,
  fetchDeploymentReleaseAudit,
...
export default function SettingsPage() {
  ...
  const [notificationCooldown, setNotificationCooldown] = useState("3600");
  const [releaseVersion, setReleaseVersion] = useState("");
  const [releaseImage, setReleaseImage] = useState("");
  const [releaseChannel, setReleaseChannel] = useState<"staging" | "production">("staging");
  ...
  const releasesQuery = useQuery({ queryKey: queryKeys.deploymentReleases({ page: 1, page_size: 100 }), queryFn: () => fetchDeploymentReleases({ page: 1, page_size: 100 }) });
  const releaseAuditQuery = useQuery({ queryKey: ["deployment-release-audit"], queryFn: fetchDeploymentReleaseAudit, refetchInterval: 30_000 });
  ...
  useEffect(() => { const notifications = objectValue(metadata.notifications); setNotificationRecipients(Array.isArray(notifications.recipients) ? notifications.recipients.join(", ") : ""); setNotificationCooldown(String(notifications.cooldown_seconds ?? 3600)); }, [deployment?.updated_at, metadata.notifications]);
  ...
  const deploymentMutation = useMutation({
    mutationFn: () => updateDeploymentSettings({ prtc_skyline_pipeline: selectedPrtc ? Number(selectedPrtc) : null, targeted_skyline_pipeline: selectedTargeted ? Number(selectedTargeted) : null, metadata: { ...metadata, diann: { ...diannMetadata, performance_tags: { threads: threads.trim() || undefined, temp: temp.trim() || undefined } }, notifications: { recipients: notificationRecipients.split(",").map((item) => item.trim()).filter(Boolean), cooldown_seconds: Number(notificationCooldown) || 3600 } } }),
  ...
  const invalidateReleases = () => queryClient.invalidateQueries({ queryKey: queryKeys.deploymentReleases({ page: 1, page_size: 100 }) });
  const releaseMutation = useMutation({ mutationFn: () => createDeploymentRelease({ version: releaseVersion.trim(), image: releaseImage.trim(), channel: releaseChannel }), onSuccess: async () => { setReleaseVersion(""); setReleaseImage(""); await invalidateReleases(); } });
  const releaseAction = useMutation({ mutationFn: async ({ id, action }: { id: number; action: "promote" | "rollout" | "verify" }) => action === "promote" ? promoteDeploymentRelease(id) : action === "rollout" ? rolloutDeploymentRelease(id, (nodesQuery.data?.results ?? []).map((node) => node.id)) : verifyDeploymentRelease(id), onSuccess: invalidateReleases });
  const notificationTest = useMutation({ mutationFn: sendNotificationTest });
  ...
  return <div className="grid gap-4">
    ...
    <Card><CardHeader><CardTitle>Global system settings</CardTitle><CardDescription>Stored in the site deployment record and applied to resolved project pipelines.</CardDescription></CardHeader><CardContent className="grid gap-4 xl:grid-cols-[1fr_1fr_1.4fr]"><Field label="DIA-NN threads"><Input value={threads} onChange={(event) => setThreads(event.target.value)} inputMode="numeric" placeholder="1" /></Field><Field label="DIA-NN temp path"><Input value={temp} onChange={(event) => setTemp(event.target.value)} placeholder="/scratch/diann" /></Field><div className="grid gap-3 sm:grid-cols-2"><Field label="PRTC Skyline pipeline"><PipelineSelect value={selectedPrtc} onChange={setSelectedPrtc} pipelines={skylinePipelines} /></Field><Field label="Targeted Skyline pipeline"><PipelineSelect value={selectedTargeted} onChange={setSelectedTargeted} pipelines={skylinePipelines} /></Field></div><div className="grid gap-3 border-t pt-3 md:grid-cols-[1fr_180px]"><Field label="Notification recipients"><Input value={notificationRecipients} onChange={(event) => setNotificationRecipients(event.target.value)} placeholder="ops@example.org, pi@example.org" /></Field><Field label="Alert cooldown (seconds)"><Input value={notificationCooldown} onChange={(event) => setNotificationCooldown(event.target.value)} inputMode="numeric" /></Field></div><div className="xl:col-span-3 flex items-center justify-between gap-3 border-t pt-3"><TagList title="Stored performance tags" tags={performanceTags} /><div className="flex gap-2"><Button variant="secondary" onClick={() => notificationTest.mutate()} disabled={notificationTest.isPending}>Send test email</Button><Button onClick={() => deploymentMutation.mutate()} disabled={deploymentMutation.isPending}><Save className="h-4 w-4" />Save global settings</Button></div></div></CardContent></Card>
    <Card><CardHeader><CardTitle>Release management</CardTitle><CardDescription>Create immutable releases, promote channels, and roll them out to registered nodes.</CardDescription></CardHeader><CardContent className="grid gap-4"><div className="grid gap-3 md:grid-cols-[1fr_1.5fr_160px_auto]"><Input placeholder="Version (e.g. 2026.09.1)" value={releaseVersion} onChange={(event) => setReleaseVersion(event.target.value)} /><Input placeholder="Container image" value={releaseImage} onChange={(event) => setReleaseImage(event.target.value)} /><select className="h-10 rounded-md border bg-background px-3 text-sm" value={releaseChannel} onChange={(event) => setReleaseChannel(event.target.value as "staging" | "production")}><option value="staging">Staging</option><option value="production">Production</option></select><Button onClick={() => releaseMutation.mutate()} disabled={!releaseVersion.trim() || !releaseImage.trim() || releaseMutation.isPending}>Create release</Button></div><div className="divide-y">{(releasesQuery.data?.results ?? []).map((release) => <div key={release.id} className="grid gap-2 py-3 md:grid-cols-[1fr_100px_1fr_auto] md:items-center"><div><div className="font-semibold">{release.version} {release.active && <span className="text-xs text-emerald-600">· active</span>}</div><div className="text-xs text-muted-foreground">{release.image}</div></div><span className="rounded border px-2 py-1 text-center text-xs uppercase">{release.channel}</span><div className="text-xs text-muted-foreground">Nodes: {(nodesQuery.data?.results ?? []).filter((node) => node.desired_release?.id === release.id).map((node) => `${node.name} (${node.release_status})`).join(", ") || "none"}</div><div className="flex flex-wrap justify-end gap-2"><Button size="sm" variant="secondary" onClick={() => releaseAction.mutate({ id: release.id, action: "promote" })}>Promote</Button><Button size="sm" onClick={() => releaseAction.mutate({ id: release.id, action: "rollout" })}>Roll out</Button><Button size="sm" variant="secondary" onClick={() => releaseAction.mutate({ id: release.id, action: "verify" })}>Verify</Button></div></div>)}</div></CardContent></Card>
    <Card><CardHeader><CardTitle>Release audit history</CardTitle><CardDescription>Recent promotions, rollouts, verification, and rollback actions.</CardDescription></CardHeader><CardContent><div className="divide-y">{(releaseAuditQuery.data ?? []).map((event) => <div key={event.id} className="flex flex-wrap justify-between gap-2 py-2 text-sm"><span>{event.message}</span><span className="text-xs text-muted-foreground">{event.actor_username || "system"} · {new Date(event.created_at).toLocaleString()}</span></div>)}</div></CardContent></Card>
